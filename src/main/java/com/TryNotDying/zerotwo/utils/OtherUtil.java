...
{
    public final static String NEW_VERSION_AVAILABLE = "There is a new version of ZeroTwo available!\n"
                    + "Current version: %s\n"
                    + "New Version: %s\n\n"
    ...
    public static void checkVersion(Prompt prompt)
    {
        ...
        {
            prompt.alert(Prompt.Level.WARNING, "ZeroTwo Version", String.format(NEW_VERSION_AVAILABLE, version, latestVersion));
        }
    }
    ...
    /**
     * Checks if the bot ZeroTwo is being run on is supported & returns the reason if it is not.
    ...
    public static String getUnsupportedBotReason(JDA jda) 
    {
        ...
        if (info.isBotPublic())
            return "\"Public Bot\" is enabled. Using ZeroTwo as a public bot is not supported. Please disable it in the "
