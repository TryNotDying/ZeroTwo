...
public class OtherUtil
{
    ...
    /**
     * Checks if Zero-Two is being run on is supported & returns the reason if it is not.
    ...
    public static String getUnsupportedBotReason(JDA jda) 
    {
        if (jda.getSelfUser().getFlags().contains(User.UserFlag.VERIFIED_BOT))
            return "Zero-Two is verified.";

        ApplicationInfo info = jda.retrieveApplicationInfo().complete();
        if (info.isBotPublic())
            return "\"Public Bot\" is enabled. Using Zero-Two as a public bot is not supported. Please disable it in the "
